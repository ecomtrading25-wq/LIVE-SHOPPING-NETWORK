...
async function startServer() {
  ...
  // Subscription webhooks need raw body for signature verification
  app.post(
    "/api/webhooks/stripe/subscriptions",
    express.raw({ type: "application/json" }),
    (req, res) => {
      const webhookService = initializeSubscriptionWebhooks();
      webhookService.processWebhook(req, res);
    }
  );
  ...
  // Xero OAuth callback
  const { xeroCallbackRouter } = await import('../xero-callback');
